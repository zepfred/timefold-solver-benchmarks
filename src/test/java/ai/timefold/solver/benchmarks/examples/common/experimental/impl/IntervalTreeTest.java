package ai.timefold.solver.benchmarks.examples.common.experimental.impl;
...
import ai.timefold.solver.benchmarks.examples.common.experimental.api.IntervalBreak;
import ai.timefold.solver.benchmarks.examples.common.experimental.api.IntervalCluster;
...
class IntervalTreeTest {
    ...
    private IntervalTree<TestInterval, Integer, Integer>
            getIntegerIntervalTree() {
        return new IntervalTree<>(TestInterval::getStart,
                TestInterval::getEnd, (a, b) -> b - a);
    }
    ...
    void testNonConsecutiveIntervals() {
        IntervalTree<TestInterval, Integer, Integer> tree =
                getIntegerIntervalTree();
        Interval<TestInterval, Integer> a =
                tree.getInterval(new TestInterval(0, 2));
        Interval<TestInterval, Integer> b =
                tree.getInterval(new TestInterval(3, 4));
        Interval<TestInterval, Integer> c =
                tree.getInterval(new TestInterval(5, 7));
        tree.add(a);
        tree.add(b);
        tree.add(c);
    ...
    void testConsecutiveIntervals() {
        ...
        Interval<TestInterval, Integer> a =
                tree.getInterval(new TestInterval(0, 2));
        Interval<TestInterval, Integer> b =
                tree.getInterval(new TestInterval(2, 4));
        Interval<TestInterval, Integer> c =
                tree.getInterval(new TestInterval(4, 7));
        tree.add(a);
        tree.add(b);
        tree.add(c);
    ...
    void testIntervalRemoval() {
        ...
        Interval<TestInterval, Integer> a =
                tree.getInterval(new TestInterval(0, 2));
        Interval<TestInterval, Integer> b =
                tree.getInterval(removedInterval);
        Interval<TestInterval, Integer> c =
                tree.getInterval(new TestInterval(4, 7));
        tree.add(a);
        tree.add(b);
    ...
    void testOverlappingInterval() {
        ...
        Interval<TestInterval, Integer> removedInterval1 =
                tree.getInterval(removedTestInterval1);
        Interval<TestInterval, Integer> c =
                tree.getInterval(new TestInterval(2, 4));

        Interval<TestInterval, Integer> d =
                tree.getInterval(new TestInterval(5, 6));

        Interval<TestInterval, Integer> e =
                tree.getInterval(new TestInterval(7, 9));
        TestInterval removedTestInterval2 = new TestInterval(7, 9);
        Interval<TestInterval, Integer> removedInterval2 =
                tree.getInterval(removedTestInterval2);
        ...
        verifyBreaks(tree);
        Interval<TestInterval, Integer> g =
                tree.getInterval(new TestInterval(6, 7));
        tree.add(g);
        clusterList = new IterableList<>(tree.getConsecutiveIntervalData().getIntervalClusters());
        assertThat(clusterList).hasSize(2);
    ...
    private static int intervalClusterCompare(IntervalCluster<TestInterval, Integer, Integer> a,
        ...
        if (!(a instanceof IntervalClusterImpl)
                || !(b instanceof IntervalClusterImpl)) {
            throw new IllegalArgumentException("Expected (" + a + ") and (" + b + ") to both be IntervalClusterImpl");
        }

        IntervalClusterImpl<TestInterval, Integer, Integer> first =
                (IntervalClusterImpl<TestInterval, Integer, Integer>) a;
        IntervalClusterImpl<TestInterval, Integer, Integer> second =
                (IntervalClusterImpl<TestInterval, Integer, Integer>) b;
    ...
    void testRandomIntervals() {
        ...
        for (int i = 0; i < 100; i++) {
            Map<TestInterval, Interval<TestInterval, Integer>> intervalToInstanceMap =
                    new HashMap<>();
            TreeSet<IntervalSplitPoint<TestInterval, Integer>> splitPoints =
                    new TreeSet<>();
            IntervalTree<TestInterval, Integer, Integer> tree =
                    new IntervalTree<>(TestInterval::getStart,
                            TestInterval::getEnd, (a, b) -> b - a);
            for (int j = 0; j < 100; j++) {
                ...
                Interval<TestInterval, Integer> interval =
                        intervalToInstanceMap.computeIfAbsent(data, tree::getInterval);
                ...
                IntervalSplitPoint<TestInterval, Integer> endSplitPoint =
                        splitPoints.floor(treeInterval.getEndSplitPoint());
                ...
                IntervalSplitPoint<TestInterval, Integer> previous =
                        null;
                IntervalSplitPoint<TestInterval, Integer> current = splitPoints.isEmpty() ? null : splitPoints.first();
                List<IntervalClusterImpl<TestInterval, Integer, Integer>> intervalClusterList =
                        new ArrayList<>();
                List<IntervalBreakImpl<TestInterval, Integer, Integer>> breakList =
                        new ArrayList<>();
                while (current != null) {
                    intervalClusterList
                            .add(new IntervalClusterImpl<>(
                    ...
                    if (previous != null) {
                        ...
                        breakList.add(new IntervalBreakImpl<>(
                                before, after, after.getStart() - before.getEnd()));
                    }
                    previous = current;
