package ai.timefold.solver.benchmarks.examples.vehiclerouting.persistence.util;
...
public class VehicleRoutingDistanceTypeComparison extends LoggingMain {
    ...
    public VehicleRoutingDistanceTypeComparison() {
        dataDir = CommonApp
                .determineDataDir(VehicleRoutingApp.DATA_DIR_NAME);
    ...
    public void compare(String... filePaths) {
        ...
        for (File varFile : files) {
            ...
            VehicleRoutingSolution variablesSolution =
                    solutionFileIO.read(varFile);
            for (File inputFile : files) {
                HardSoftLongScore score;
                if (inputFile == varFile) {
                    score = variablesSolution.getScore();
                } else {
                    VehicleRoutingSolution inputSolution =
                            solutionFileIO.read(inputFile);
    ...
    private void applyVariables(
            VehicleRoutingSolution inputSolution,
            VehicleRoutingSolution varSolution) {
        List<Vehicle> inputVehicleList =
                inputSolution.getVehicleList();
        Map<Long, Vehicle> inputVehicleMap =
                new LinkedHashMap<>(inputVehicleList.size());
        for (Vehicle vehicle : inputVehicleList) {
            inputVehicleMap.put(vehicle.getId(), vehicle);
        }
        List<Customer> inputCustomerList =
                inputSolution.getCustomerList();
        Map<Long, Customer> inputCustomerMap =
                new LinkedHashMap<>(inputCustomerList.size());
        for (Customer customer : inputCustomerList) {
            inputCustomerMap.put(customer.getId(), customer);
        }
        for (Customer varCustomer : varSolution
                .getCustomerList()) {
